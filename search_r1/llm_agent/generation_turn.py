...
class LLMGenerationManager:
    ...
    def _batch_tokenize(self, responses: List[str]) -> torch.Tensor:
        ...
        return self.tokenizer(
            ...
            return_tensors='pt', 
            padding="longest" # 按当前 batch 最长序列的长度做右侧 padding
        )['input_ids']
    ...
    def _process_next_obs(self, next_obs: List[str], dones: List[str]) -> torch.Tensor:
    ...
    def run_llm_loop(self, gen_batch, initial_input_ids: torch.Tensor) -> Tuple[Dict, Dict]:
        ...
        for step in range(self.config.max_turns):
            ...
            next_obs_ids = self._process_next_obs(next_obs, dones)
            # for seq in next_obs_ids:
            #     detok = tokenizer.decode(seq, skip_special_tokens=True)
            #     if "</information>" not in detok:
            #         print("⚠️  </information> 被截断！")
            #         print("decode:", repr(detok))            
            # Update states
