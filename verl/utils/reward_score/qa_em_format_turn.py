...
def is_valid_search_part(part_text):
    ...
    for part in parts:
        ...
        if re.match(r"</?(?:think|search)>", part):
            # 标签逻辑
            if part == "<think>" and state in ["start", "after_think"]:
                state = "in_think"
...
def compute_score_em(solution_str, ground_truth, new_parts, method='strict', structure_format_score=0, final_format_score=0, retrieval_score=0, format_score=0, score=1.):
    ...
    if answer is None:
        ...
        if em_check(answer, ground_truth['target']):
            answer_score = (score-structure_format_score) * score
        else:
            answer_score = final_format_score
    
    for i in even_indices:
        if step_scores[i] == 1:
            step_scores[i] = structure_format_score / (count_ones+count_zeros) if count_ones+count_zeros > 0 else 0
        else:
            step_scores[i] = 0.0 
