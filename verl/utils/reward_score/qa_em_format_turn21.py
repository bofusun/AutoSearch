...
def compute_score_em(solution_str, ground_truth, new_parts, per_turn_infos, method='strict', support_docs=None, structure_format_score=0, final_format_score=0, retrieval_score=0, format_score=0, score=1.):
    ...
    answer_f1_score = query_f1_score(prediction=answer, golden_answers=ground_truth['target'])
    ...
    for i in range(len(step_valids)-1):
        if step_valids[i] == 1:
            ...
            # step_scores[i] = structure_format_score
        else:
            # step_scores[i] = 0
            step_scores[i] = -0.05

    # 逐步提取每一回合的 <information> 内容，如果没有则为空串
    information_matches = extract_information_contents(per_turn_infos)
    # print("information_matches:",information_matches)
    # # 这里假设 step_information_gain 的返回长度与 information_matches 一致
    # print("support_docs:",support_docs)
    information_gains, redundancy_penalty = step_information_gain(informations=information_matches, golden_info=support_docs)
    novelty_rewards = [0.6*(gain -penalty) for gain, penalty in zip(information_gains, redundancy_penalty)]
    # novelty_rewards = [0.6*gain for gain in information_gains]
    ...
    if do_print:
        ...
        print(f"Answer_f1_score: {answer_f1_score}")
        print(f"information_matches: {information_matches}")
        print(f"support_docs: {support_docs}")
        print(f"Information gains: {information_gains}")
        print(f"Redundancy penalty: {redundancy_penalty}")
        print(f"Novelty_rewards: {novelty_rewards}")
