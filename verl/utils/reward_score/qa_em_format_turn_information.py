...
# def step_information_gain(informations: list[list[str]]) -> tuple[list[float], list[float]]:

#     if len(informations) == 0:
#         return []

#     redundancy_penalty = [0.0 for _ in informations]
    
#     # 保存已经出现过的句子
#     seen_info = set()
    
#     for i, current_infos in enumerate(informations):
#         # ========== 计算重复惩罚 ==========
#         for info in current_infos:
#             norm_info = normalize_answer(info)
#             if norm_info in seen_info:
#                 redundancy_penalty[i] = 1.0
#             else:
#                 seen_info.add(norm_info)
...
def compute_score_em(solution_str, ground_truth, new_parts, method='strict', format_score=0., score=1.0):
    ...
    for i, part in enumerate(new_parts):
        ...
        step_scores_all.append(1 if valid else 0) 

    # 先统计偶数项中为1和为0的个数
    even_indices = list(range(0, len(step_scores_all), 2))
    count_ones = sum(step_scores_all)
    count_zeros = len(even_indices) - count_ones

    # 再更新偶数项的值
    for i in even_indices:
        if step_scores_all[i] == 1:
            step_scores[i] = 0.2 / (count_ones+count_zeros) if count_ones+count_zeros > 0 else 0
        else:
            step_scores[i] = 0.0 / count_zeros if count_zeros > 0 else 0

    # ---------- 2. 冗余惩罚：只在偶数且 valid==1 时检查 i+1 ----------
    seen_info = set()

    for i in range(0, len(new_parts), 2):          # 只扫偶数项
        if step_scores_all[i] != 1:                # 必须 valid
            continue
        if i + 1 >= len(new_parts):                # 下一个 part 不存在
            continue

        next_part = new_parts[i + 1]
        # 提取 <information>…</information> 块
        info_blocks = re.findall(r'\n\n<information>(.*?)</information>\n\n',
                                next_part, flags=re.DOTALL)

        penalty = 0.0

        norm = normalize_answer(info_blocks.strip())   # 整块归一化
        if norm in seen_info:                    # 曾经出现过
            penalty = -0.01
        else:
            penalty = 0.01
        seen_info.add(norm)

        step_scores[i] += penalty
    # seen_info = set()
    # for i in range(0, len(new_parts), 2):         # 只扫偶数项
    #     if step_scores_all[i] != 1:                   # 必须 valid
    #         continue
    ...
    #     next_part = new_parts[i + 1]
    #     # 提取所有 <information> … </information> 块
    #     info_blocks = re.findall(r'\n\n<information>(.*?)</information>\n\n',
    #                              next_part, flags=re.DOTALL)
    #     penalty = 0.0
    #     for block in info_blocks:
    #         for line in block.splitlines():
    #             norm = normalize_answer(line.strip())
    #             if not norm:
    #                 continue
    #             if norm in seen_info:               # 出现重复
    #                 penalty = 0.01
    #                 break
    #             seen_info.add(norm)
    #         if penalty == 0.01:
    ...
    # for i in even_indices:
    #     if step_scores[i] == 1:
    #         step_scores[i] = 0.1 / count_ones if count_ones > 0 else 0
    #     else:
    #         step_scores[i] = 0.0 / count_zeros if count_zeros > 0 else 0
